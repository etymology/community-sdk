...
class EInkDisplay {
 public:
  ...
  bool refreshDisplayAsync(RefreshMode mode = FAST_REFRESH, bool turnOffScreen = false);
 ...
 private:
  ...
  bool ramDataEntryConfigured;
  bool ramAreaConfigured;
  uint16_t ramAreaX;
  uint16_t ramAreaY;
  uint16_t ramAreaW;
  uint16_t ramAreaH;
  ...
  void sendCommandData(uint8_t command, const uint8_t* data, uint32_t length);
